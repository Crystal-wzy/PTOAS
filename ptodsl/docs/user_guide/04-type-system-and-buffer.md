...
`pto.f8e8m0` is storage-only. Use it for MX scale storage, including an L1 source pointer passed to `pto.load_cbuf_to_ca_mx` or `pto.load_cbuf_to_cb_mx`; it is not a scalar arithmetic type.
