...
def main() -> None:
    ...
    for (label, compiled, module_spec_override), observation, launch_target_arch in zip(
        ...
        expected_fatobj = native_build_runtime._requires_fatobj(compiled.mlir_text())
        expect(
            observation["fatobj"] == expected_fatobj,
            f"{label} native build should request ptoas fatobj emission only when required by the MLIR",
        )
        expect(
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        expect(
            "--fatobj" in ptoas_cmd,
            "native build should pass --fatobj when ptoas is responsible for fatobj emission",
        )
        expect(
        ...
        expect(
            ...
            "native build should not pass --fatobj unless fatobj emission is explicitly requested",
        )
        expect(
