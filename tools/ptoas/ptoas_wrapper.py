...
_CONFIGURED_MLIR_PYTHON_ROOT = Path("@MLIR_PYTHON_PACKAGE_DIR@")
...
def _configured_mlir_python_root() -> Path | None:
    root_text = str(_CONFIGURED_MLIR_PYTHON_ROOT)
    if not root_text or root_text.startswith("@"):
        return None
    root = _CONFIGURED_MLIR_PYTHON_ROOT
    if (root / "mlir" / "ir.py").is_file():
        return root.resolve()
    return None


def _prepend_python_path(root: Path) -> None:
    root_text = str(root)
    ...
    if root_text in parts:
        parts.remove(root_text)
...
def _bootstrap_python_path(wrapper: Path) -> None:
    if _is_build_tree_wrapper(wrapper):
        ...
        if build_root is not None:
            ...
            mlir_root = _configured_mlir_python_root()
            if mlir_root is not None:
                _prepend_python_path(mlir_root)
            _prepend_python_path(root.resolve())
            return
        raise SystemExit("unable to locate the build-tree layout for ptoas")
...
def main() -> None:
    ...
    _check_archive_python_requirement(wrapper)
